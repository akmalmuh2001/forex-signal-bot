...
def generate_signal(df):
    ...
    # --- Konfirmasi Entry ---
    has_liq_sweep = len(liq_signals) > 0
    has_smc       = len(smc_signals) > 0

    if "BUY" in signal:
        if has_liq_sweep and last["rsi"] < 45:
            entry_type   = "🔥 STRONG BUY - Konfirmasi Liquidity Sweep + RSI"
            entry_action = "✅ Entry sekarang (Market Order / Buy Limit di harga ini)"
        elif has_smc:
            entry_type   = "✅ BUY - Konfirmasi SMC"
            entry_action = "⚠️ Tunggu candle M15 berikutnya close dulu sebelum entry"
        else:
            entry_type   = "🟡 BUY LEMAH - Indikator saja"
            entry_action = "⏳ Sebaiknya tunggu konfirmasi tambahan"
    elif "SELL" in signal:
        if has_liq_sweep and last["rsi"] > 55:
            entry_type   = "🔥 STRONG SELL - Konfirmasi Liquidity Sweep + RSI"
            entry_action = "✅ Entry sekarang (Market Order / Sell Limit di harga ini)"
        elif has_smc:
            entry_type   = "✅ SELL - Konfirmasi SMC"
            entry_action = "⚠️ Tunggu candle M15 berikutnya close dulu sebelum entry"
        else:
            entry_type   = "🟡 SELL LEMAH - Indikator saja"
            entry_action = "⏳ Sebaiknya tunggu konfirmasi tambahan"
    else:
        entry_type   = "⏸️ Tidak ada sinyal kuat"
        entry_action = "⏳ Tunggu setup yang lebih jelas"
